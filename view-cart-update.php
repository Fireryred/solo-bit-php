<?php 
	...
	if ($dbError){
		...
		$product_id = $_POST['id'];
		$query = "
			UPDATE order_details SET qty=? WHERE product_id=? AND order_id=?;
		";
		$stmt = $db->prepare($query);
		$stmt->bind_param('iii', $qty, $product_id, $order_id);
		$stmt->execute();
		logMessage("Update quantity of product id ".$product_id." from cart by user: ".$_SESSION['username']);
